...
export const updateLicenseImage = (user, imageBlob) => (dispatch) => async (firebase) => {
  ...
  const {
    ...
    await driverDocsRef(user.id).put(imageBlob);
    let image = await driverDocsRef(user.id).getDownloadURL();
      let profile = user;
